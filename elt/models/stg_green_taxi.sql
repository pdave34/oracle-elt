...
SELECT
    ...
    cast(to_char(EHAIL_FEE) as number) as EHAIL_FEE,
    ...
    cast(to_char(TRIP_TYPE) as number) as TRIP_TYPE,
    cast(to_char(CONGESTION_SURCHARGE) as number) as CONGESTION_SURCHARGE
    FROM {{ source('pdave', 'RAW_GREEN_TRIPDATA') }}
),
...
renamed AS (
    ...
    SELECT
        ...
        CASE WHEN L1."EHAIL_FEE" < 0 THEN 0 ELSE L1."EHAIL_FEE" END AS ehail_fee
    FROM L1
